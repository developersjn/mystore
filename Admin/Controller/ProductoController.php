...
if (isset($_POST["operacion"])) {
    ...
    if ($operacion == "Create") {
        ...
        $objProductoController->CreateWithAjax();
    }
} else {
    require "../../Model/Dao/ProductoDao.php";
}
...
class ProductoController {
    ...
    public function CreateWithAjax() {
        ...
        $directorioImgPrincipal = "../View/upload/img/producto";
        if (!is_dir($directorioImgPrincipal)) {
            mkdir($directorioImgPrincipal);
        }

        $rutaImgPrinciapl = $this->EscribirImagenDirectorio($tmp_name_img_principal, $type_img_principal, $name_img_principal, $directorioImgPrincipal);
        $rutaImagenBD=str_replace("../", "",$rutaImgPrinciapl);       
        ...
        $objProducto->setIdCategoria($idCategoria);
        $objProducto->setImagenPortada($rutaImagenBD);

        //guardar producto en la tabla
        $objProductoDao = new ProductoDao();
        $IdProductoRegistrado = $objProductoDao->Create($objProducto);

        $directorioGaleria = "../View/upload/img/producto/" . $IdProductoRegistrado;
        if (!is_dir($directorioGaleria)) {
            mkdir($directorioGaleria);
        }

        $objGaleriaDao = new GaleriaDao();
        ...
        for ($i = 0; $i < count($tmp_name_sec); $i++) {
            ...
            $rutaImgGaleria = $this->EscribirImagenDirectorio($tmp_name, $type, $name, $directorioGaleria);
            $rutaImagenGaleriaBD=str_replace("../", "",$rutaImgGaleria); 
            
            $objGaleria = new Galeria();
            $objGaleria->setUrlFoto($rutaImagenGaleriaBD);
            $objGaleria->setIdProducto($IdProductoRegistrado);
            $objGaleriaDao->CreateWithProcedure($objGaleria);
        }
    }
